 ...
 */
@CaseDatabaseFormat(extension = "dbc", name = "ElviraDB") public class ElviraDataBaseIO
	...
	public CaseDatabase load(String fileName) throws IOException {
		...
		try {
			...
			e.printStackTrace();
			throw new IOException("RecognitionException open DBFile: " + fileName);
		} catch (TokenStreamException e) {
			e.printStackTrace();
			throw new IOException("TokenStreamException in file: " + fileName);
		} finally {
			if (fileStream != null) {
				fileStream.close();
			}
	 ...
	 * @throws IOException IOException
	 */
	public void save(String fileName, int[][] cases, List<Variable> variables)
			throws IOException {
		FileWriter writer;
		PrintWriter out;
	 ...
	 * @param cases
	 */
	private void save(PrintWriter out, List<Variable> variables, int[][] cases) throws UnknownNetworkTypeException {
	 ...
	 */
	private void writePreamble(PrintWriter out, HashMap<String, String> infoNet, int numCases)
			throws UnknownNetworkTypeException {
		// preamble comment
		out.println("//	   Network");
