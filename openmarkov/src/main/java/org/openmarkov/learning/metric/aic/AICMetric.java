...
@MetricType(name = "AIC") public class AICMetric extends EntropyMetric {
	...
	@Override protected double score(AddLinkEdit edition, boolean change) {
		...
		lastNodeDimension = nodesDimensions.get(((Node) destinationNode).getName());
	...
	@Override protected double score(InvertLinkEdit edition, boolean change) {
		...
		lastNodeDimension = nodesDimensions.get(((Node) initialDestinationNode).getName());
		newNodeDimension = lastNodeDimension / ((edition.getVariable1()).getNumStates());
