...
public class StatisticalUtilities {
	...
	static public double gser(double a, double x) {
		...
		if (x <= 0.0) {
			if (x < 0.0)
				System.out.println("Error: Parameter x is less than 0.0 in routine gser in LogFactorial");
			gammser = 0.0;
			return gammser;
		} else {
			ap = a;
	...
	static public double gcf(double a, double x) {
		...
		if (i > ITMAX)
			System.out.println("Parameter a is too large, ITMAX too small in routine gcf");
